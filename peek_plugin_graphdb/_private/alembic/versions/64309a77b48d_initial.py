...
def upgrade():
    op.execute(CreateSequence(Sequence('LiveDbItem_id_seq',  minvalue=0, schema='pl_graphdb')))
    ...
    sa.Column('propsJson', sa.String(length=500), nullable=True),
    sa.ForeignKeyConstraint(['modelSetId'], ['pl_graphdb.LiveDbModelSet.id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id'),
    schema='pl_graphdb'
    )
    op.create_index('idx_LiveDbDKey_importHash', 'LiveDbItem', ['importHash'], unique=False, schema='pl_graphdb')
    op.create_index('idx_LiveDbDKey_liveDbKey', 'LiveDbItem', ['key'], unique=False, schema='pl_graphdb')
    op.create_index('idx_LiveDbDKey_modelSetId', 'LiveDbItem', ['modelSetId'], unique=False, schema='pl_graphdb')
...
def downgrade():
    ...
    op.drop_index('idx_LiveDbDKey_modelSetId', table_name='LiveDbItem', schema='pl_graphdb')
    op.drop_index('idx_LiveDbDKey_liveDbKey', table_name='LiveDbItem', schema='pl_graphdb')
    op.drop_index('idx_LiveDbDKey_importHash', table_name='LiveDbItem', schema='pl_graphdb')
    op.drop_table('LiveDbItem', schema='pl_graphdb')
    op.drop_table('Setting', schema='pl_graphdb')
    op.drop_table('LiveDbModelSet', schema='pl_graphdb')
    # ### end Alembic commands ###