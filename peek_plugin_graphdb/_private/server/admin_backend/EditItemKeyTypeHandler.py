...
filtKey = {"key": "admin.Edit.ItemKeyType"}
...
class __ExtUpdateObservable(OrmCrudHandlerExtension):
    ...
    def _tellObserver(self, tuple_, tuples, session, payloadFilt):
        self._tupleDataObserver.notifyOfTupleUpdate(
            TupleSelector(ItemKeyType.tupleName(), {})
        )
        return True
...
def makeItemKeyTypeHandler(tupleObservable, dbSessionCreator):
    handler = __CrudHandler(dbSessionCreator, ItemKeyType,
                            filtKey, retreiveAll=True)

    logger.debug("Started")
    handler.addExtension(ItemKeyType, __ExtUpdateObservable(tupleObservable))
    return handler
